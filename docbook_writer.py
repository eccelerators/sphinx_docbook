...
class DocBookTranslator(nodes.NodeVisitor):
    ...
    def _add_processing_instruction(self, processing_instruction_name):
        """Add a processing_instruction."""
        pi = etree.ProcessingInstruction(processing_instruction_name)

        self.tb.addprevious(pi)
    ...
    def visit_colspec(self, node):
        ...
        if node.hasattr('colwidth'):
            sibling_num = node.parent.children.index(node)
            attribs['colnum'] = str(sibling_num + 1)
            attribs['colwidth'] = str(int(node['colwidth'] * 3.5))
            attribs['colname'] = 'col' + str(sibling_num + 1)
    ...
    def visit_entry(self, node):             
        ...
        for sibling in node.parent.children:
            if node == sibling:
                break
            if sibling.hasattr('morecols'):
                r += sibling['morecols']
            else:
                r += 1
        
        if node.hasattr('morecols'):
            attribs['namest'] = 'col' + str(r)
            attribs['nameend'] = 'col' + str(r + node['morecols'])

        self._push_element('entry', attribs)
